...
def _painel_topico(cid, status_atual, prioridade_atual, tipo_atual):
    ...
    if _pdf_key in st.session_state:
        ...
        with col_pdf:
            st.download_button(
                ...
                file_name=f"historico_{st.session_state.get(_pdf_nome_key, cid)}.pdf",
                mime="application/pdf",
                key=f"pdf_dl_{cid}",
                use_container_width=True)
    st.divider()

    # ── A. Histórico de interações ────────────────────────────────────────
    st.markdown("**📋 Histórico de interações**")
    ints = query("""
        SELECT ci.interacao_id, ci.data_interacao, ci.via_comunicacao,
    ...
    if not ints:
        ...
        for irow in ints:
            iid, data_i, via, pessoa, desc, result, fup_i = (
                irow["interacao_id"], irow["data_interacao"],
                irow["via_comunicacao"], irow["contato_pessoa"],
                irow["descricao"], irow["resultado"], irow["data_followup"])
            via_ico = VIA_ICONE.get(via, "📌")
            lbl = f"{via_ico} **{data_i}** — {via or '—'}"
            if pessoa: lbl += f" | {pessoa}"

            _ekey = f"exp_inter_{cid}_{iid}"
            with st.expander(lbl, expanded=False):
                if desc:   st.markdown(f"**O que foi tratado:**\n\n{desc}")
                if result: st.markdown(f"**Resultado / próximo passo:**\n\n{result}")
                if fup_i:  st.caption(f"📅 Próximo contato: {fup_i}")

                # Editar interação
                if st.button("✏️ Editar", key=f"ed_int_{cid}_{iid}",
                             use_container_width=False):
                    st.session_state[_ekey] = not st.session_state.get(_ekey, False)
                    st.rerun()

                if st.session_state.get(_ekey):
                    with st.form(f"form_ed_int_{cid}_{iid}"):
                        e_data  = st.date_input("Data", value=date.fromisoformat(data_i) if data_i else date.today())
                        e_via   = st.selectbox("Via", VIAS, index=VIAS.index(via) if via in VIAS else 0)
                        e_pess  = st.text_input("Contato / pessoa", value=pessoa or "")
                        e_desc  = st.text_area("O que foi tratado", value=desc or "", height=120)
                        e_res   = st.text_area("Resultado / próximo passo", value=result or "", height=80)
                        e_fup   = st.date_input("Próximo contato", value=date.fromisoformat(fup_i) if fup_i else None)
                    ...
                    if salvar_e:
                        ...
                        conn.execute("""UPDATE contato_interacao SET
                            ...
                            WHERE interacao_id=?""",
                            (e_data.isoformat(), e_via, e_pess or None,
                             e_desc or None, e_res or None,
                             e_fup.isoformat() if e_fup else None, iid))
                        conn.commit(); conn.close()
    ...
    _mk = f"ni_modo_{cid}"
    if st.session_state.get(_mk) != "form":
        if st.button("📝 Nova interação", key=f"btn_ni_{cid}", type="primary"):
            st.session_state[_mk] = "form"
            st.rerun()
    else:
        with st.form(f"form_ni_{cid}"):
            col1, col2 = st.columns(2)
            with col1:
                ni_data = st.date_input("Data", value=date.today(),
                                        key=f"ncn_{cid}")
                ni_via  = st.selectbox("Via", VIAS, key=f"ncc_{cid}")
            with col2:
                ni_pess = st.text_input("Contato / pessoa", key=f"ncf_{cid}")
                ni_fup  = st.date_input("Próximo contato", value=None,
                                        key=f"ncw_{cid}")
            ni_desc = st.text_area("O que foi tratado", height=120, key=f"nce_{cid}")
            ni_res  = st.text_area("Resultado / próximo passo", height=80,
                                   key=f"ncr_{cid}")
            col_s, col_c = st.columns(2)
            salvar_ni   = col_s.form_submit_button("💾 Salvar", type="primary",
                                                    use_container_width=True)
            cancelar_ni = col_c.form_submit_button("✖ Cancelar",
                                                    use_container_width=True)
        ...
        if salvar_ni:
            if not ni_desc.strip():
                st.warning("Descreva o que foi tratado antes de salvar.")
            else:
                conn = conectar()
                conn.execute("""INSERT INTO contato_interacao
                    (contato_id, data_interacao, via_comunicacao, contato_pessoa,
                     descricao, resultado, data_followup, ativo)
                    VALUES (?,?,?,?,?,?,?,1)""",
                    (cid, ni_data.isoformat(), ni_via, ni_pess or None,
                     ni_desc.strip(), ni_res.strip() or None,
                     ni_fup.isoformat() if ni_fup else None))
                # Atualiza data_followup e status no tópico
                if ni_fup:
                    conn.execute("""UPDATE contato_registro
                        SET data_followup=? WHERE contato_id=?""",
                        (ni_fup.isoformat(), cid))
                conn.commit(); conn.close()

                for k in [f"ncn_{cid}", f"ncc_{cid}", f"ncf_{cid}",
                          f"ncw_{cid}", f"nce_{cid}", f"ncr_{cid}"]:
                    st.session_state.pop(k, None)
                st.session_state[_mk] = "sel"
                st.session_state.pop(f"exp_inter_{cid}", None)
                st.session_state.pop(_pdf_key, None)
                st.session_state.pop(_pdf_nome_key, None)
                st.session_state.pop("ct_topico_aberto", None)
    ...
    # ── C. Editar tópico ──────────────────────────────────────────────────
    _kf = f"ct_edit_topico_{cid}"
    _hash_key = f"ct_edit_hash_{cid}"
    if st.button("⚙️ Editar dados do tópico", key=f"btn_ed_top_{cid}"):
        st.session_state[_kf] = not st.session_state.get(_kf, False)
        st.rerun()

    if st.session_state.get(_kf):
        cr = query("""SELECT assunto, tipo_topico, status, prioridade,
                             data_contato, data_followup, observacao,
                             cliente_id, fornecedor_id, tipo_entidade
                      FROM contato_registro WHERE contato_id=?""", (cid,))
        if cr:
            r = cr[0]
            with st.form(f"form_ed_top_{cid}"):
                assunto_e = st.text_input("Assunto", value=r["assunto"] or "")
                col1, col2, col3 = st.columns(3)
                with col1:
                    tipo_e = st.selectbox("Tipo", TIPO_TOPICO,
                        index=TIPO_TOPICO.index(r["tipo_topico"]) if r["tipo_topico"] in TIPO_TOPICO else 0)
                with col2:
                    status_e = st.selectbox("Status", STATUS_TOPICO,
                        index=STATUS_TOPICO.index(r["status"]) if r["status"] in STATUS_TOPICO else 0)
                with col3:
                    prior_e = st.selectbox("Prioridade", PRIOR,
                        index=PRIOR.index(r["prioridade"]) if r["prioridade"] in PRIOR else 1)
                col4, col5 = st.columns(2)
                with col4:
                    dc_e = st.date_input("Data início",
                        value=date.fromisoformat(r["data_contato"]) if r["data_contato"] else date.today())
                with col5:
                    fup_e = st.date_input("Próximo contato",
                        value=date.fromisoformat(r["data_followup"]) if r["data_followup"] else None)
                obs_e = st.text_area("Observação", value=r["observacao"] or "")
                salvar_top = st.form_submit_button("💾 Salvar tópico", type="primary")
            ...
            if salvar_top:
                ...
                conn.execute("""UPDATE contato_registro SET
                    ...
                    data_contato=?, data_followup=?, observacao=?
                    WHERE contato_id=?""",
                    (assunto_e, tipo_e, status_e, prior_e,
                     dc_e.isoformat(), fup_e.isoformat() if fup_e else None,
                     obs_e or None, cid))
                conn.commit(); conn.close()
                st.session_state.pop(_kf, None)
