...
def _campo_navegacao(pq_id, forn_id):
    """Navegacao por categoria - implementacao em segunda instancia."""
    pass
...
def _tela_detalhe(pq_id):
    ...
    if cab_salvo_msg:
        st.success(f"✅ Cabeçalho salvo: {cab_salvo_msg}")

    # Expander de correcao de cabecalho — fecha apos salvar
    _exp_aberto = not cab_salvo_msg and st.session_state.get(f"pq_cab_exp_{pq_id}", False)
    lbl_exp = "✏️ Corrigir cabeçalho da pesquisa"
    with st.expander(lbl_exp, expanded=_exp_aberto):
        if not st.session_state.get(f"pq_cab_exp_{pq_id}"):
            st.session_state[f"pq_cab_exp_{pq_id}"] = True
        _form_editar_cabecalho_pesquisa(pq_id, cli_id_a, forn_id_a,
                                            pdv_id_a, data_a, obs_a)
        if st.button("✖ Fechar edição", key=f"fechar_cab_{pq_id}"):
            st.session_state.pop(f"editar_cab_{pq_id}", None); st.rerun()
